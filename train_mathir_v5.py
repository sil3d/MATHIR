...
def train_with_fixes(config_path):
    ...
    for episode in range(config['training']['num_episodes']):
        ...
        # In real use, this comes from the env.
        # Batch size B=32, Feature dim D=256
        observations = torch.randn(config['training']['batch_size'], config['model']['feature_dim'])
        
        # Apply domain randomization
        observations = dr_manager.augment_batch(observations, training=True)
