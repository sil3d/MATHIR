...
""")
    ...
    print(f"{C.BOLD}── Recommended: point your agent at the universal proxy ──{C.RESET}")
    print(f"{C.DIM}  What you just installed (MCP tools + instruction-file injection) still")
    print(f"  works, but it's the fallback path. If your agent supports a custom LLM")
    print(f"  base URL, prefer the proxy instead: no config-file edits, works")
