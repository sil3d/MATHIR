...
class MATHIRMemory(nn.Module):
    ...
    def __init__(self, input_dim=256, mem_slots=512):
        ...
        self.register_buffer(
            'episodic_values',
            torch.zeros(mem_slots, input_dim)
        )
        self.episodic_ptr = 0
        ...
        # Learned projection for semantic → full dim
        self.semantic_proj = nn.Linear(64, input_dim)
    ...
    def forward(self, x, actions=None, step=0):
        ...
        if actions is not None:
            ...
            with torch.no_grad():
                ...
                self.episodic_values[idx] = x.mean(0).detach()
        ...
        x_semantic = self.semantic_proj(x_semantic)
    ...
    def update_working_memory(self, x):
        ...
        with torch.no_grad():
            indices = (self.working_ptr + torch.arange(batch_size, device=x.device)) % self.working_capacity
    ...
    def update_semantic_prototypes(self, x):
        ...
        with torch.no_grad():
            x_downsampled = F.adaptive_avg_pool1d(x.unsqueeze(1), 64).squeeze(1)
            distances = torch.cdist(x_downsampled, self.semantic_prototypes)
            closest = distances.argmin(dim=1)
            alpha = 0.01
            # Batch update using scatter
            for i in range(x.size(0)):
                self.semantic_prototypes[closest[i]] = (
                    (1 - alpha) * self.semantic_prototypes[closest[i]] + 
                    alpha * x_downsampled[i]
                )
                self.semantic_usage[closest[i]] += 1
