...
class WorkingMemory(nn.Module):
    ...
    def retrieve(self, features):
        return features # Identity for now
...
class HierarchicalMemoryCore(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Initialize memories (same as before)
        self.working_memory = WorkingMemory(config.get('working_capacity', 64))
        self.episodic_memory = EpisodicMemory(config.get('episodic_capacity', 1000))
        self.semantic_memory = SemanticMemory(config.get('semantic_prototypes', 256))
        self.immunological_memory = ImmunologicalMemory(
            config.get('immunological_capacity', 100)
        )
    ...
    def forward(self, features, prev_router_weights=None):
        ...
        # Retrieve from memories (simplified)
        contexts = []
        if w_work.sum() > 0.1:
        ...
        if w_imm.sum() > 0.1:
            ...
            contexts.append(immune_resp * w_imm if immune_resp is not None else 0)
        
        # Combine contexts
        # Note: if contexts is empty, which can happen if weights are very erratic or zeroed (unlikely due to softmax)
        # But safeguards are good.
        if contexts:
            # assuming same shape, sum them
            combined = sum(contexts)
            # handle case where immune_resp return 0 integer
...
class DomainRandomizationManager:
    ...
    def __init__(self, config):
        ...
        if self.mode in ['offline', 'hybrid']:
            ...
            if pool_path:
                try:
                    self.offline_pool = torch.load(pool_path)
                except:
