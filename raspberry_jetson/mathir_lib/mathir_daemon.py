...
from mathir_mcp_server import (
    ...
    handle_memory_recall, handle_memory_save,
    ...
    try:
        ...
        while True:
            ...
            try:
                if method == 'memory_recall':
                ...
                elif method == 'memory_hybrid_search':
                    from mathir_vec import VecMemory
                    from mathir_search import rrf_fusion, _tokenize
                    ...
                    if db_path is None:
                        ...
                        # Detect schema
                        cursor = dconn.execute("PRAGMA table_info(memories)")
                        ...
                        if _has_vec:
                            ...
                            rows = dconn.execute(sql, params_list).fetchall()
                            for row in rows:
                        ...
                        for mid, rrf_score in fused[:k]:
                            meta = dconn.execute(
                                f"SELECT {text_col}, tier, timestamp "
                            ...
                            if meta:
                                ...
                                if 'agent' in columns:
                                    agent_val = dconn.execute(
                                        f"SELECT agent FROM memories WHERE memory_id = ?", [mid]
                                    ).fetchone()[0] or ''
                                else:
                                    try:
                                        ...
                                        if meta_row and meta_row[0]:
                                            md = json.loads(meta_row[0])
                                            agent_val = md.get('agent', '')
                                    except Exception:
                                        pass
                                results.append({
                                    'memory_id': mid,
                        ...
                        dconn.close()
                        result = {'results': results, 'query': query_text, 'total': len(results),
                    ...
                    try:
                        ...
                        log.error(f"PING ERROR: {type(e).__name__}: {e}", exc_info=True)
                        raise
                else:
