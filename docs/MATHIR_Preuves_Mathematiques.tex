...
\section{Résultats Expérimentaux V3 (Step 212k)}
La comparaison sur 212 000 pas de temps démontre :
\begin{itemize}
    \item \textbf{LSTM (Dynamic LR) :} Score Rétention $\to 0.0$ après $t=210k$ (Arrêt Cardiaque Cognitif).
    \item \textbf{MATHIR V3 (Plasticity) :} Score Rétention $\to 1.0$. Le système a surmonté le "mauvais démarrage" initial pour atteindre une stabilité asymptotique parfaite.
\end{itemize}

\section{Confirmation Longue Durée (Step 210k)}
L'expérience prolongée jusqu'à $t=210,000$ a révélé une divergence critique finale :
\begin{itemize}
    \item \textbf{LSTM ("Arrêt Cardiaque")} : À $t \approx 210,400$, le LSTM subit une chute brutale de performance (Gradient Crash), prouvant son instabilité structurelle à très long terme malgré un score apparent élevé (overfitting).
    \item \textbf{MATHIR (Robustesse)} : Aucune dégradation n'est observée. La rétention reste stable à $1.0$, confirmant que la combinaison \textit{Mémoire Séparée + mHC} offre une solution pérenne pour la conduite autonome continue.
\end{itemize}
