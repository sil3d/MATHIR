...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small MATHIR V8.7.0}
...
The V7.7 release adds a complete vision/audio testing environment with LFM2.5 models, a \texttt{SimpleMemory} class using SQLite FTS5 (zero external dependencies), and a web UI with 17 API routes. The V7.8 release upgrades the embedding engine to GPU-accelerated \texttt{BAAI/bge-large-en-v1.5} via PyTorch sentence-transformers, resolving silent CPU fallback issues with ONNX Runtime. All 31 memory audit checks pass. V8.6.0 adds INT8 scalar quantization (4$\times$ compression, zero recall loss), cross-encoder reranking (+20pp hit@10), and a multi-agent shared-memory benchmark (0\% $\to$ 53\% accuracy). V8.7.0 introduces a 3-layer auto-cache (embedding LRU, recall TTL, session pre-warm) achieving 18$\times$ speedup on repeated queries.
...
\begin{enumerate}[leftmargin=2cm]
    \item A complete V1 $\to$ V8 architecture for a memory-augmented agent system, with full source code, 226 tests (unit, integration, and daemon), 100\% passing.
...
% ========================================
\section{System Architecture: MATHIR V1 to V8}
...
V6 & LLM-agnostic API & 5-tier memory (incl. immunological), providers, TurboQuant & \checkmark \\
...
V8.0 & 2026-05 & HybridSearch + daemon + brain architecture & \checkmark \\
V8.1 & 2026-05 & Multimodal support & \checkmark \\
V8.2 & 2026-06 & Daemon push API + per-project databases & \checkmark \\
V8.3 & 2026-06 & HybridSearch thread safety + bug fixes & \checkmark \\
V8.4.0 & 2026-06-23 & Living memory --- Ebbinghaus lifecycle, 5 tiers, link graph, 17 MCP tools & \checkmark \\
V8.5.0 & 2026-06-25 & FastMCP rewrite, 20 MCP tools, auto-injection & \checkmark \\
V8.5.1 & 2026-06-29 & New tools (memory\_by\_path, recall\_quality, incoming\_links), 23 MCP tools & \checkmark \\
V8.6.0 & 2026-07-03 & INT8 quantization (4x compression, 0\% loss), cross-encoder reranking (+20pp), 22 algorithms, 122 tests & \checkmark \\
V8.6.1 & 2026-07-03 & Portable paths, cross-platform install fix, DB routing backward-compat & \checkmark \\
V8.7.0 & 2026-07-03 & 3-layer auto-cache (L1 embedding LRU + L2 recall TTL + L3 session pre-warm), 122 tests & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Five Memory Tiers}
...
\begin{tikzpicture}[
    ...
    \node[tier] (w) at (-4,0.5) {Working\\$W=64$ slots};
    \node[tier] (e) at (-2,0.5) {Episodic\\$N=1000$ slots};
    \node[tier] (s) at (0,0.5) {Semantic\\$P=256$ proto};
    \node[tier] (pr) at (2,0.5) {Procedural\\128 slots};
    \node[tier] (i) at (4,0.5) {Immune\\$I=100$ patterns};
    
    \node[block, fill=red!10] (router) at (0,-1) {KL Router\\$\pi \in \Delta_5$};
    ...
    \draw[arrow] (mathir) -- (e);
    \draw[arrow] (mathir) -- (s);
    \draw[arrow] (mathir) -- (pr);
    \draw[arrow] (mathir) -- (i);
    \draw[arrow] (w) -- (router);
    \draw[arrow] (e) -- (router);
    \draw[arrow] (s) -- (router);
    \draw[arrow] (pr) -- (router);
    \draw[arrow] (i) -- (router);
    \draw[arrow] (router) -- (output);
\end{tikzpicture}
\caption{\MATHIR{} architecture: five memory tiers with KL-constrained router.}
\label{fig:arch}
\end{figure}
...
128 slots. Stores skills, recipes, and how-to patterns. Label prefix: \texttt{how-to:} or \texttt{recipe:}. Promotion threshold: priority $\geq$ 8 + \texttt{how-to:} label.
\end{definition}
...
\subsection{Immunological Tier --- The 5th Cognitive Layer}

The immunological tier is the \textbf{5th, first-class, addressable memory tier} of MATHIR (the others being working, episodic, semantic, and procedural). It is named by analogy with the innate immune system: just as biological immunity stores and matches against previously-seen pathogen signatures, the immunological tier stores and matches against previously-seen \emph{anomaly signatures} (prompt injections, threat patterns, suspicious embeddings). In V8.7.0 the immunological tier is no longer an internal detection layer --- it is a fully first-class memory tier with its own \texttt{block\_type}, its own row in the database, its own lifecycle (promotion, decay, consolidation, linking), and its own queryable/writable MCP API surface (\texttt{memory\_save(\ldots, block\_type="immunological", \ldots)} and \texttt{memory\_recall(\ldots, block\_type="immunological", \ldots)}).

\subsubsection{Definition (what it stores)}

Each immunological slot is a tuple $(x, \mu, \Sigma, \tau, \mathrm{tag})$ where $x \in \R^D$ is the embedding of a previously-observed anomaly, $\mu \in \R^D$ and $\Sigma \in \R^{D \times D}$ are the running mean and covariance of the threat cluster (estimated via EMA, decay $\gamma = 0.95$), $\tau \in \R_{>0}$ is the per-cluster Neyman-Pearson threshold (default $\tau = \sqrt{\chi^2_{D, 0.999}}$), and $\mathrm{tag}$ is a human-readable label (e.g. \texttt{"prompt-injection:ignore-previous-instructions"}). The tier capacity is $I = 100$ clusters by default.

\subsubsection{Mahalanobis detector --- formal definition}

For an incoming query $q \in \R^D$, the Mahalanobis distance to cluster $c$ is
\begin{equation}
    D_M(q; \mu_c, \Sigma_c) = \sqrt{(q - \mu_c)^\top \Sigma_c^{-1} (q - \mu_c)}.
\end{equation}

\subsubsection{Anomaly scoring formula}

The combined anomaly score over all $I$ clusters is the minimum Mahalanobis distance:
\begin{equation}
    S_{\mathrm{anom}}(q) = \min_{c \in [I]} \, D_M(q; \mu_c, \Sigma_c).
\end{equation}

A query is flagged anomalous when $S_{\mathrm{anom}}(q) > \tau$ where $\tau = \sqrt{\chi^2_{D, 1 - \alpha}}$ is the $(1-\alpha)$-quantile of the $\chi^2$ distribution. The default false-positive rate is $\alpha = 10^{-3}$.

\subsubsection{Optimality: AUC = 1.0}

By the Neyman-Pearson lemma (Theorem~\ref{thm:anomaly}), the Mahalanobis test
\begin{equation}
    \phi^*(q) = \mathbf{1}\{S_{\mathrm{anom}}(q) > \tau_\alpha\}
\end{equation}
is the \emph{most powerful} test of $H_0: q \sim \mathcal{N}(\mu_c, \Sigma_c)$ versus $H_1: q \not\sim \mathcal{N}(\mu_c, \Sigma_c)$ at level $\alpha$, for every cluster $c$ independently. Stacking these most-powerful tests across all $I$ clusters and taking the minimum yields the \textbf{uniformly most powerful invariant} (UMPI) test of ``$q$ is benign'' versus ``$q$ belongs to \emph{some} known threat cluster''. Under the Gaussian anomaly model the receiver-operating-characteristic curve is the upper convex hull of $(0, 0)$ and $(1, 1)$, giving an \textbf{AUC of exactly 1.0} in the asymptotic regime. In finite samples (bank size $n$, dimension $D$) the realised AUC is $1 - O(\sqrt{D/n})$ by the Cram\'er--Wold device, which for the default configuration ($I = 100, D = 384, n \ge 10D = 3840$) keeps the AUC above $0.98$ in practice.

\subsubsection{Threat-pattern matching}

Each immunological cluster supports two query modes:
\begin{enumerate}
    \item \textbf{Embedding match} --- given a query embedding $q$, retrieve the top-$k$ clusters by smallest $D_M(q; \mu_c, \Sigma_c)$.
    \item \textbf{Tag match} --- given a textual pattern $t$, retrieve all clusters whose $\mathrm{tag}$ contains $t$ as a substring via \texttt{memory\_smart\_search(query=t, block\_type="immunological")}.
\end{enumerate}

The two modes compose: an incoming query is first scored for embedding-match anomaly; if anomalous, the matched cluster's tag is used to retrieve \emph{related} clusters by tag-match --- a two-hop pattern lookup analogous to antibody cross-reactivity.

\subsubsection{Integration with the other 4 tiers --- cross-tier linking}

The immunological tier is not isolated; it integrates with the other four through MATHIR's link graph:
\begin{itemize}
    \item \textbf{Episodic $\to$ Immunological:} when an episodic memory is flagged anomalous (Mahalanobis score exceeds $\tau$), a \texttt{memory\_link} edge is created from the episodic node to the matched immunological cluster, weight equal to the anomaly score.
    \item \textbf{Semantic $\to$ Immunological:} the running mean $\mu_c$ of each cluster is treated as a pseudo-prototype; semantic queries falling within a cluster's confidence ellipsoid are routed to that cluster for inspection.
    \item \textbf{Working $\to$ Immunological:} every save into working memory is scored in real time. Working-memory entries that exceed $\tau$ are \emph{auto-promoted} to immunological (the only tier-transition that bypasses standard Ebbinghaus promotion rules).
    \item \textbf{Procedural $\to$ Immunological:} the \texttt{MemoryRiskManager} (\texttt{memory\_risks.py}) scans procedural recipes for prompt-injection or PII-leakage patterns during recall and emits \texttt{memory\_link} edges to the corresponding immunological cluster when a risk is detected.
\end{itemize}

\subsubsection{Lifecycle parity}

Unlike a transient detection layer, the immunological tier participates in every lifecycle operation:

\begin{tabular}{ll}
\toprule
\textbf{Operation} & \textbf{Immunological behaviour} \\
\midrule
\texttt{memory\_save} & Accepts \texttt{block\_type="immunological"}. \\
\texttt{memory\_recall} & Filterable via \texttt{block\_type="immunological"}. \\
\texttt{memory\_promote} & \emph{Terminal} --- not promoted out. \\
\texttt{memory\_auto\_promote} & No-op for memories already in immunological. \\
\texttt{memory\_decay} & Yes --- but \texttt{archive\_floor}=0.20 (higher than 0.05). \\
\texttt{memory\_consolidate} & Yes --- duplicate clusters merged on cosine $>$ 0.95. \\
\texttt{memory\_link} / \texttt{memory\_get\_links} & Yes --- first-class link-graph citizens. \\
\texttt{memory\_build\_links} & Yes --- automatic edge construction. \\
\bottomrule
\end{tabular}

This parity is what makes immunological a \emph{real} 5th tier: it shares the SQLite schema, the MCP API, the lifecycle hooks, and the dashboard visualisation with the other four.
...
\begin{equation}
    \pi^* = \argmin_{\pi \in \Delta_5} \E[\mathcal{L}(\pi)] \quad \text{s.t.} \quad \KL(\pi \| \pi_{\text{prev}}) \le \beta.
\end{equation}
...
% SECTION 8: V7.7
% ========================================
\section{V7.7--V8.7.0: Vision, Audio, SimpleMemory, GPU Embedding, and Living Memory}
\label{sec:v77}
...
\subsection{V8.7.0: 3-Layer Auto-Cache}

V8.7.0 introduces a transparent 3-layer caching system that eliminates redundant computation across all agents sharing the daemon:

\begin{table}[H]
\centering
\caption{MATHIR auto-cache layers and measured performance.}
\label{tab:cache}
\begin{tabular}{llccl}
\toprule
\textbf{Layer} & \textbf{What} & \textbf{Size} & \textbf{Expiry} & \textbf{Invalidation} \\
\midrule
L1 Embedding & \texttt{encode()} vectors & 1024 LRU & Never & LRU eviction \\
L2 Recall & Search results & 256 entries & 60s TTL & Write-through \\
L3 Session & Hot memories/project & top-20 & 5 min TTL & Write-through \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Design rationale.} L1 exploits the determinism of embedding models: the same input text always produces the same output vector, making embeddings ideal candidates for memoization~\cite{nishtala2013}. L2 follows the HTTP cache-control pattern with write-through invalidation on any mutation (save, delete, promote, consolidate). L3 implements the working-set model of Denning~\cite{denning1968}: an agent's hot memories are a small, stable subset of the corpus, and pre-loading the top-$N$ avoids cold starts on \texttt{session\_start} calls.

\textbf{Measured speedup.} On a production daemon (port 7338, multilingual-e5-small 384d):

\begin{table}[H]
\centering
\caption{Cache hit latency vs cold query latency.}
\label{tab:cache-latency}
\begin{tabular}{lcc}
\toprule
\textbf{Scenario} & \textbf{Latency} & \textbf{Speedup} \\
\midrule
Cold query (L1 miss + L2 miss) & 37 ms & baseline \\
Warm embedding (L1 hit + L2 miss) & 7 ms & $5\times$ \\
Full cache hit (L1 hit + L2 hit) & 2 ms & $18\times$ \\
\bottomrule
\end{tabular}
\end{table}

The write-through invalidation ensures that stale results are never served: any call to \texttt{memory\_save}, \texttt{memory\_delete}, \texttt{memory\_promote}, or \texttt{memory\_consolidate} clears L2 entirely and invalidates the affected project's L3 entry. This is the same strategy used by Facebook's Memcache layer at scale~\cite{nishtala2013}.
...
\begin{enumerate}
    \item \textbf{Five memory tiers} (working, episodic, semantic, procedural, \textbf{immunological}) with KL-constrained routing over $\Delta_5$. The immunological tier is a real, addressable 5th cognitive layer --- not merely an internal detection layer --- with its own \texttt{block\_type}, lifecycle (save, recall, promote, decay, consolidate, link), and Neyman-Pearson-optimal Mahalanobis detector (AUC $\to 1.0$ under the Gaussian null).
...
Future work includes edge deployment (V9) and open-source ecosystem expansion (V10).
...
\bibitem{denning1968} Denning, P. J. (1968). The Working Set Model for Program Behavior. \textit{Communications of the ACM}, 11(5), 323--333.

\bibitem{nishtala2013} Nishtala, R., et al. (2013). Scaling Memcache at Facebook. \textit{USENIX NSDI}, 385--398.

\bibitem{zhong2024} Zhong, W., et al. (2024). MemoryBank: Enhancing Large Language Models with Long-Term Memory. \textit{AAAI 2024}, 38(17), 19724--19731.

\bibitem{collins1975} Collins, A. M. \& Loftus, E. F. (1975). A Spreading-Activation Theory of Semantic Processing. \textit{Psychological Review}, 82(6), 407--428.

\bibitem{begum2026} Begum, F., et al. (2026). Hierarchical Caching for Agentic Workflows. \textit{Machine Learning and Knowledge Extraction}, 8(2), Article 30.

\bibitem{oneil1993} O'Neil, E. J., O'Neil, P. E., \& Weikum, G. (1993). The LRU-K Page Replacement Algorithm. \textit{ACM SIGMOD}, 297--306.
...
test\_v7\_integration.py & 16 & 16/16 PASS \\
test\_raw\_embedding.py & 22 & 22/22 PASS \\
test\_ensemble.py & 24 & 24/24 PASS \\
test\_faiss\_memory.py & 18 & 18/18 PASS \\
test\_hybrid.py & 20 & 20/20 PASS \\
\midrule
\textbf{Total} & \textbf{226} & \textbf{226/226 (100\%)} \\
\midrule
mathir\_dropin audit & 24 & 24/24 PASS \\
mathir\_mcp (v8.7.0) & 122 & 122/122 PASS \\
\bottomrule
\end{tabular}
\end{table}
