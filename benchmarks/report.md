...
5. **Hybrid BM25 fusion + cross-encoder rerank** (pre-existing, `multi_dataset_efficient.py`) — both lose to plain dense-only on scifact/nfcorpus with a strong embedder (bge-base-en-v1.5), but hybrid fusion actually WINS with the current weaker embedder (see 3b) — same underlying pattern.

**The one consistent finding across all five**: any secondary/augmentation signal (BM25, CE, PRF) helps when the baseline dense signal is weak and hurts when it's already strong. The two purely-corrective techniques (hubness, anisotropy) show no reliable win at any tested setting. **I did not find a mathematical breakthrough this session** — I ruled out five plausible directions with real evidence instead of leaving them as unverified assumptions. That's the honest state of things.
...
- **Re-test all 5 rejected techniques (section 4) with a STRONGER embedder** (e.g. e5-small or bge-base) instead of the current weak one. The consistent "helps weak / hurts strong" pattern predicts several of these might flip to a clean win once the baseline dense signal is already strong — this was never tested, only inferred from the pattern. Cheap to test (no API needed), high potential value.
