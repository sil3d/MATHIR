...
- **DONE (with a correction, not a clean confirmation), but the follow-up question is still open**: RRF hybrid fusion and PRF were re-tested with `intfloat/multilingual-e5-small` (`benchmarks/07_utilities/retest_with_stronger_embedder.py`) expecting the "helps weak / hurts strong" pattern to flip. It did NOT flip for hybrid fusion (still helps, even though e5-small is much stronger than the default) — but `bge-base-en-v1.5` (stronger still) DOES show hybrid hurting. **Real open question**: characterize what specifically differs between e5-small and bge-base-en-v1.5 that causes this (score distribution shape? calibration? something else?) — re-test the other 3 rejected techniques (hubness, anisotropy, confidence-gating) with e5-small and bge-base too, since only hybrid-fusion and PRF have been re-tested with a stronger embedder so far.
