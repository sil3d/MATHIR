...
<div class="footer">
  Generated by MATHIR v8.4.0 benchmark suite &middot;
...
def render_index(reports: list, title: str = "MATHIR v8.4.0 — Benchmark Suite") -> str:
    """reports = list of (filename, label, summary_line)"""
    html = [_html_head(title)]
    html.append(_header(
        title,
        "All v8.4.0 lifecycle benchmarks in one place &middot; click a report to open",
        [("v8.4.0", "badge-blue"), ("living memory", "badge-green")],
    ))
...
def main():
    ...
    title = args.title or f"MATHIR v8.4.0 benchmark — {json_path.stem}"
